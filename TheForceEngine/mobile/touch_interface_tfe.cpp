...
void TouchInterface::mouseMove(int action, float x, float y, float mouse_x, float mouse_y)
{
    ...
    // The DOS-style in-game menus (escape menu, agent menu, briefing) have large
    // buttons, so we drive the cursor by teleporting it straight to the finger and
    // tapping to click. The ImGui frontend menus have small widgets, so there we
    // keep the relative drag-the-cursor behaviour for fine positioning. Both report
    // TS_MENU to us, hence the engine-side PortableInGameMenu() query.
    // mobile_screen_width/height are the device (== SDL window) pixels, which is the
    // coordinate space the engine maps the absolute mouse position from. These menus
    // render in a centred box, so the engine also hands back the horizontal pillarbox
    // offset to subtract so the cursor lands under the finger.
    float menuOffsetX = 0;
    ...
    else if(action == TOUCHMOUSE_TAP)
    {
        if(tapMode)
        {
            // Teleport the cursor onto the tapped button before clicking.
            MouseMoveAbsolute(absX, absY);
        }
...
void TouchInterface::createControls(std::string filesPath)
{
    ...
    tcGameMain->addControl(new touchcontrols::Button("map", touchcontrols::RectF(2, 0, 4, 2), "map", PORT_ACT_DATAPAD, false, false, "Open map (PDA)"));
